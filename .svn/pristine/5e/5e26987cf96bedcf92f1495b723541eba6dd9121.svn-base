...
(function (factory) {
	...
	Tree.prototype = {
		...
		//liaochente 新增打开所有文件目录 start
		//根据ID选中节点
		selectedId: function (o) {
			var $treeEl = o.el;
			var item = o.item;
			//$treeEl, item, $parentEl
			var $parentEl = $treeEl;
			var $itemEl = $parentEl.find("i.hid_node").filter(function (_, treeItem) {
					return $(treeItem).attr("v") == item && !$(treeItem).parent().parent().parent().is(".tree-selected");
		...
		discloseFolder: function discloseFolder( el ) {
			...
			$branch.find( '> .tree-branch-header .icon-folder' ).eq( 0 )
				...
				.removeClass( ' tree-plus' )//LIAOCHENTE ACE
				.addClass( 'tree-minus' );//LIAOCHENTE ACE
		...
		//discloseAll
		discloseAll: function discloseAll(_call) {
			var self = this;
