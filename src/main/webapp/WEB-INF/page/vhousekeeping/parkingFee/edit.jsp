...
<h3 class="header smaller lighter blue">编辑停车费配置信息</h3>
<div class="row">
	<div class="col-xs-12">
		<!-- PAGE CONTENT BEGINS -->
		<form class="form-horizontal data-form" role="form" action="edit/exec" enctype="multipart/form-data" valid = "true">
				...
				<div class="col-sm-9">
					<select name="villageId.id" class="form-control" style="width:41.6%">
						<c:forEach items="${villages }" var="village">
							<option <c:if test="${PAGE_DATA.villageId.id eq village.id }">selected='selected'</c:if> value="${village.id }">${village.villageName }</option>
						</c:forEach>
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<div class="clearfix">
						<input type="text" name="chargeType" value="${PAGE_DATA.chargeType }" class="col-xs-10 col-sm-5" />
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<div class="clearfix">
						<input type="text" name="chargeNumber" value="${PAGE_DATA.chargeNumber }" class="col-xs-10 col-sm-5" placeholder="元/月" />
					</div>
				</div>
			</div>
