...
<div class="page-header">
	<h1>
		修改 <small> <i class="ace-icon fa fa-angle-double-right"></i>
			修改物业管理员信息
		</small>
	</h1>
</div>
...
<div class="row">
	<div class="col-xs-12">
		<!-- PAGE CONTENT BEGINS -->
		<form class="form-horizontal data-form" role="form" valid = "true" action="${pageContext.request.contextPath}/vestate/estate/estateUserManage/edit/exec" method="post">

			<input type="hidden" id="id" name="id" value="${PAGE_DATA.id }">
			<input type="hidden" id="salt" name="salt" value="${PAGE_DATA.salt}">
			<input type="hidden" id="username" name="username" value="${PAGE_DATA.username}">
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
				<div class="clearfix">
					<input type="text" id="nickName" placeholder="昵称" class="col-xs-10 col-sm-5" name="nickName" value="${PAGE_DATA.nickName }"/>
				</div>
				</div>
			</div>
			...
			<div class="col-sm-9">
				...
				<c:forEach var="item" items="${village}" varStatus="vs">
					<option value="${item.villageId}"  <c:if test="${item.flag == 'true'}"> selected='selected'</c:if> >${item.villageName}</option>
				</c:forEach>
				</select>
			</div>
			</div>
