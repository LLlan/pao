...
<div class="row">
	<div class="col-xs-12">
		<!-- PAGE CONTENT BEGINS -->
		<form class="form-horizontal data-form" role="form" enctype="multipart/form-data" action="edit/exec" valid = "true">
					...
					<div class="clearfix">
						<select name="vid" class="form-control select" >
							...
							<c:forEach items="${villages }" var="village">
								<option <c:if test="${PAGE_DATA.anTypeInfo.village.id eq village.id }">selected='selected'</c:if> value="${village.id }">${village.villageName }</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="col-sm-9">
					<div class="clearfix">
						<select name="typeId" class="form-control select" >
							<option value="${PAGE_DATA.anTypeInfo.id }">${PAGE_DATA.anTypeInfo.typeName }</option>
						</select>
					</div>
				</div>
			</div>
