...
(function($ , undefined) {
	...
	var Ace_File_Input = function(element , settings) {
		...
		this.disabled = false;
		this.can_reset = true;
		//liaochente 绑定 格式校验、扩展名、文件大小校验 错误回调
		this.$element.on("file.error.ace", function(e, data) {
			var errorInfos = [];
			var error_list = data["error_list"];
			var error_count = data["error_count"];
			for(var v in error_count) {
				var length = error_count[v];
				var files = error_list[v];
				for(var i=0; i < length; i++) {
					errorInfos.push(files[i] + data["info_type_zh"][v]);
				}
			}
			if(settings.emm_error) {
				settings.emm_error(e, $(element), errorInfos, data);
			} else {
				var html = '<ul class="list-unstyled spaced">';
				for(var i=0; i<errorInfos.length; i++) {
					html += '<li class="text-danger">';
					html += '<i class="ace-icon fa fa-times bigger-110 red"></i>' + errorInfos[i];
					html += '</li>';
				}
				html += '</ul>';
				$.messager.alert(html);
	...
	var checkFileList = function(files, dropped) {
		...
		this.$element.trigger(
			event = new $.Event('file.error.ace'), 
			{
				...
				'dropped': dropped,
				'info_type_zh' : {'ext' : "扩展名应为" + this.settings.allowExt, 'mime' : "类型应为" + this.settings.allowMime ,"size":"大小不能超过" + this.settings.maxSize/1024 + "KB"}
			}
		);
	...
	$.fn.ace_file_input.defaults = {
		...
		denyMime: null,
		maxSize: false,
		//liaochente 增加格式校验、大小校验、后缀名校验错误回调
		emm_error:null,
		//callbacks
		before_change: null,
		before_remove: null,
